...
- [Chapter 5: Prototypes](#chapter-5-prototypes)
    - [`[[Prototype]]`](#prototype)
        ...
        - [Setting & Shadowing Properties](#setting--shadowing-properties)
    - ["Class"](#class)
        - ["Class" Functions](#class-functions)
            - [What's in a name?（那我们怎么叫这种行为呢）](#whats-in-a-name那我们怎么叫这种行为呢)
        - ["Constructors"](#constructors)
            - [Constructor Or Call?](#constructor-or-call)
        - [Mechanics](#mechanics)
            - ["Constructor" Redux](#constructor-redux)
    - ["(Prototypal) Inheritance"](#prototypal-inheritance)
        - [Inspecting "Class" Relationships](#inspecting-class-relationships)
    - [Object Links](#object-links)
        - [`Create()`ing Links](#createing-links)
            - [`Object.create()` Polyfilled](#objectcreate-polyfilled)
        - [Links As Fallbacks?](#links-as-fallbacks)
...
几乎就javascript能叫做oo，因为这是少数能直接创建对象的。==！
...
小结，我们通过new来构建对象与对象之间的联系（即建立连接），这种行为在语言表述上(new)和实际行为(建立连接)的不一致，导致了一些副作用。

#### What's in a name?（那我们怎么叫这种行为呢）
...
只有在使用new的时候，这个调用才叫做constructors call,new后面的函数仅仅是普通函数，并不是constructors.
...
这样（=）只是创建了Foo的另一个引用，如果你改变Bar.prototype，这会改变Foo.prototype，所以，这并不是你想要的。
