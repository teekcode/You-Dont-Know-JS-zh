...
在第二章，我们说了用callbacks来表达异步流控制的两个主要的缺点：

* 基于回调的异步流不符合我们大脑计划一个任务的步骤。
...
在多线程可抢占的语言里面，`bar()`是可能在`foo()`的语句中间执行的。但js不是抢占式的语言，也不是多线程的。但是，如果`foo()`能够主动配合地停下来来迎合这种打断，那么`bar()`就可能在`foo()`里面执行。
