e00b41b  on 3 Nov 2016
@abroz abroz Grammar - why clause SV order
...
- [Chapter 5: Grammar](#chapter-5-grammar)
    - [Statements & Expressions](#statements--expressions)
        - [Statement Completion Values(语句执行完成后返回的值)](#statement-completion-values语句执行完成后返回的值)
        - [Expression Side Effects(表达式的副作用)](#expression-side-effects表达式的副作用)
        - [Contextual Rules](#contextual-rules)
            ...
            - [`else if` And Optional Blocks](#else-if-and-optional-blocks)
    - [Operator Precedence](#operator-precedence)
        - [Short Circuited](#short-circuited)
        - [Tighter Binding](#tighter-binding)
        - [Associativity](#associativity)
        - [Disambiguation](#disambiguation)
    - [Automatic Semicolons(自动加分号)](#automatic-semicolons自动加分号)
        - [Error Correction](#error-correction)
    - [Errors](#errors)
        - [Using Variables Too Early](#using-variables-too-early)
...
我们这册书最后的话题是js的语法。你也许认为你会写js，但这里有很多坑，会导致疑惑和误解，让我们来弄清楚。
...
### Statement Completion Values(语句执行完成后返回的值)
...
In fact, if you've done much code experimenting in your console (or in a JavaScript environment REPL -- read/evaluate/print/loop tool), you've probably seen `undefined` reported after many different statements, and perhaps never realized why or what that was. 想的简单点，控制台只是报告了语句的完成值。
...
在es7里面有个提议，叫做"do expression",可以这样用：
```js
var a, b;
...
大体的思想是，我们把语句看作表达式 -- they can show up inside other statements -- without needing to wrap them in an inline function expression and perform an explicit `return ..`.
...
### Expression Side Effects(表达式的副作用)
...
很多开发者认为，这里的`{ .. }`只是单独的表示`object` literal，并不会赋值。但不是的。

在这里，`{ .. }`只是平常的代码片段，这样用不符合js的规范，但是合法的。用块作用于声明的`let`中，这样写特别有用。(详见*Scope & Closures*)
...
## Automatic Semicolons(自动加分号)

ASI (Automatic Semicolon Insertion) 特性是js会自动在你写的程序里面加入应该有的分号，即使你没有写。
...
需要注意的是ASI特性只会在新的一行之前加入分号，而不会在行中加分号。
